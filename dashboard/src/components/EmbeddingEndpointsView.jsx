...
function HealthHistogram({ history, width = 120, height = 24 }) {
  ...
  const barWidth = Math.max(2, Math.min(4, Math.floor(width / buckets.length)));
...
function HealthDetailModal({ isOpen, onClose, healthData }) {
  ...
  return (
    <Modal title={`Health: ${healthData.EndpointName}`} onClose={onClose}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
          <div><strong>Status:</strong> <span className={`status-badge ${healthData.IsHealthy ? 'active' : 'inactive'}`}>{healthData.IsHealthy ? 'Healthy' : 'Unhealthy'}</span></div>
          <div><strong>Uptime:</strong> {uptimePct}</div>
          <div><strong>History:</strong> {spanStr}</div>
        </div>
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
          <div><strong>Consecutive OK:</strong> {healthData.ConsecutiveSuccesses}</div>
          <div><strong>Consecutive Fail:</strong> {healthData.ConsecutiveFailures}</div>
        </div>
        {healthData.LastError && (
          <div><strong>Last Error:</strong> <span style={{ color: '#f44336' }}>{healthData.LastError}</span></div>
        )}
        <div>
          <strong>24h Histogram:</strong>
          <div style={{ marginTop: 8 }}>
            <HealthHistogram history={history} width={500} height={32} />
          </div>
        </div>
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', fontSize: '0.85em', color: '#888' }}>
          <div>First check: {healthData.FirstCheckUtc ? new Date(healthData.FirstCheckUtc).toLocaleString() : 'N/A'}</div>
          <div>Last check: {healthData.LastCheckUtc ? new Date(healthData.LastCheckUtc).toLocaleString() : 'N/A'}</div>
          <div>Last healthy: {healthData.LastHealthyUtc ? new Date(healthData.LastHealthyUtc).toLocaleString() : 'N/A'}</div>
          <div>Last unhealthy: {healthData.LastUnhealthyUtc ? new Date(healthData.LastUnhealthyUtc).toLocaleString() : 'N/A'}</div>
        </div>
      </div>
    </Modal>
  );
