namespace Test.Shared
{
    ...
    /// <summary>
    /// Minimal Gemini-compatible test server used to simulate concurrent upstream calls.
    /// </summary>
    public sealed class SlowGeminiCompatibleServer : IDisposable, IAsyncDisposable
    {
        ...
        public string CompletionModel { get; set; } = "gemini-2.5-flash";
        ...
        public SlowGeminiCompatibleServer(int embeddingDelayMs = 0, int completionDelayMs = 0)
        {
        ...
        private async Task HandleClientAsync(TcpClient client, CancellationToken token)
        {
            ...
            using (NetworkStream stream = client.GetStream())
            {
                try
                {
                    ...
                    if (request.Method == "POST" && request.Path.Contains(":generateContent", StringComparison.Ordinal))
                    {
                    ...
                    if (request.Method == "POST" && request.Path.Contains(":batchEmbedContents", StringComparison.Ordinal))
                    {
                        Interlocked.Increment(ref _EmbeddingRequestCount);
                        await DelayIfNeededAsync(EmbeddingDelayMs, token).ConfigureAwait(false);
                        await WriteJsonResponseAsync(stream, 200, BuildBatchEmbeddingResponse(request.Body)).ConfigureAwait(false);
                        return;
                    }

                    if (request.Method == "POST" && request.Path.Contains(":embedContent", StringComparison.Ordinal))
                    {
                        Interlocked.Increment(ref _EmbeddingRequestCount);
                        await DelayIfNeededAsync(EmbeddingDelayMs, token).ConfigureAwait(false);
                        await WriteJsonResponseAsync(stream, 200, new
        ...
        private static object BuildBatchEmbeddingResponse(string requestBody)
        {
            int requestCount = CountBatchEmbeddingInputs(requestBody);
            List<object> embeddings = new List<object>();
        ...
        private static int CountBatchEmbeddingInputs(string requestBody)
        {
            ...
            try
            {
                ...
                if (!doc.RootElement.TryGetProperty("requests", out JsonElement requests))
                    return 1;

                if (requests.ValueKind == JsonValueKind.Array)
                    return Math.Max(1, requests.GetArrayLength());
