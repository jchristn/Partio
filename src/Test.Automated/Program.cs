    ...
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            using (PartioClient user = new PartioClient(_Endpoint, _TestToken))
            {
                ...
                {
                    if (string.IsNullOrEmpty(geminiCepId)) return;
                    await admin.DeleteCompletionEndpointAsync(geminiCepId);
                });

                await RunTest("Delete vLLM Completion Endpoint", async () =>
                {
                    if (string.IsNullOrEmpty(vllmCepId)) return;
                    await admin.DeleteCompletionEndpointAsync(vllmCepId);
                });
                ...
                {
                    if (string.IsNullOrEmpty(geminiEpId)) return;
                    await admin.DeleteEndpointAsync(geminiEpId);
                });

                await RunTest("Delete vLLM Embedding Endpoint", async () =>
                {
                    if (string.IsNullOrEmpty(vllmEpId)) return;
                    await admin.DeleteEndpointAsync(vllmEpId);
                });
