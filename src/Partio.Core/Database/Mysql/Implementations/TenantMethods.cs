namespace Partio.Core.Database.Mysql.Implementations
{
    ...
    public class TenantMethods : ITenantMethods
    {
        ...
        public TenantMethods(MysqlDatabaseDriver driver, LoggingModule logging)
        {
        ...
        public async Task<TenantMetadata?> ReadByIdAsync(string id, CancellationToken token = default)
        {
            ...
            string query = "SELECT * FROM tenants WHERE id = '" + _Driver.Sanitize(id) + "' LIMIT 1;";
        ...
        public async Task<EnumerationResult<TenantMetadata>> EnumerateAsync(EnumerationRequest request, CancellationToken token = default)
        {
            ...
            string dataQuery = "SELECT * FROM tenants " + whereClause.ToString() + " " + orderByClause + " LIMIT " + fetchCount + ";";
