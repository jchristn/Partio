namespace Partio.Core.Database.Sqlserver.Implementations
{
    ...
    public class UserMethods : IUserMethods
    {
        ...
        public UserMethods(SqlServerDatabaseDriver driver, LoggingModule logging)
        {
        ...
        public async Task<UserMaster?> ReadByIdAsync(string id, CancellationToken token = default)
        {
            ...
            string query = "SELECT TOP 1 * FROM users WHERE id = '" + _Driver.Sanitize(id) + "';";
        ...
        public async Task<UserMaster?> ReadByEmailAsync(string tenantId, string email, CancellationToken token = default)
        {
            ...
            string query = "SELECT TOP 1 * FROM users WHERE tenant_id = '" + _Driver.Sanitize(tenantId) + "' AND email = '" + _Driver.Sanitize(email) + "';";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        public async Task<EnumerationResult<UserMaster>> EnumerateAsync(string tenantId, EnumerationRequest request, CancellationToken token = default)
        {
            ...
            string dataQuery = "SELECT TOP " + fetchCount + " * FROM users " + whereClause + " " + orderByClause + ";";
