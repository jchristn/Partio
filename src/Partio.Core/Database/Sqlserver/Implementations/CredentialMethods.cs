namespace Partio.Core.Database.Sqlserver.Implementations
{
    ...
    public class CredentialMethods : ICredentialMethods
    {
        ...
        public CredentialMethods(SqlServerDatabaseDriver driver, LoggingModule logging)
        {
        ...
        public async Task<Credential?> ReadByIdAsync(string id, CancellationToken token = default)
        {
            ...
            string query = "SELECT TOP 1 * FROM credentials WHERE id = '" + _Driver.Sanitize(id) + "';";
        ...
        public async Task<Credential?> ReadByBearerTokenAsync(string bearerToken, CancellationToken token = default)
        {
            ...
            string query = "SELECT TOP 1 * FROM credentials WHERE bearer_token = '" + _Driver.Sanitize(bearerToken) + "';";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        public async Task<EnumerationResult<Credential>> EnumerateAsync(string tenantId, EnumerationRequest request, CancellationToken token = default)
        {
            ...
            string dataQuery = "SELECT TOP " + fetchCount + " * FROM credentials " + whereClause + " " + orderByClause + ";";
