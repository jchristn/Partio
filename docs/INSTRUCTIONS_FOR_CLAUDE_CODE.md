<!-- Paste this into your CLAUDE.md or system prompt to teach the agent how to use Partio over MCP. -->

# Using the Partio MCP server (Claude Code)

## Connecting

Point Claude Code at the Partio MCP server once, then the tools are available in every session.

The quickest path is the Partio installer, which writes the Claude Code config for you:

```bash
partio-mcp mcp install
```

Or add it yourself with the Claude Code CLI:

```bash
claude mcp add --transport http --scope user partio http://localhost:8500/mcp
```

Either way you end up with an entry in `~/.claude.json`:

```json
"mcpServers": {
  "partio": { "type": "http", "url": "http://localhost:8500/mcp" }
}
```

If the server requires a bearer token, supply it the way Claude Code expects for an HTTP MCP server (an `Authorization: Bearer <token>` header). The server rejects unauthenticated requests with `401` before any tool runs.
