<!-- Paste this into your Cursor rules / system prompt to teach the agent how to use Partio over MCP. -->

# Using the Partio MCP server (Cursor)

## Connecting

Cursor reads MCP servers from a **project-scoped** file, `.cursor/mcp.json`, in your working directory. Add Partio as an HTTP server:

```json
{
  "mcpServers": {
    "partio": { "url": "http://localhost:8500/mcp", "transport": "http" }
  }
}
```

The Partio installer writes this file for you (into the current directory's `.cursor/mcp.json`):

```bash
partio-mcp mcp install
```

Because the config is project-scoped, each project you want Partio in needs its own `.cursor/mcp.json`. If the server requires a bearer token, supply it through Cursor's HTTP MCP auth-header mechanism (`Authorization: Bearer <token>`); unauthenticated requests are rejected with `401` before any tool runs. Reload the MCP servers in Cursor and confirm the Partio tools appear.
