<!-- Paste this into your Codex system prompt / config to teach the agent how to use Partio over MCP. -->

# Using the Partio MCP server (Codex)

## Connecting

Codex connects to Partio over the **stdio bridge** rather than an HTTP URL. Register it with the Codex CLI:

```bash
codex mcp add partio -- partio-mcp mcp stdio
```

This tells Codex to launch `partio-mcp mcp stdio` as a subprocess and speak JSON-RPC over its stdin/stdout. The bridge relays each call to the Partio HTTP server for you, which also means it is the right choice when a bearer token is required — the token lives in the bridge's own configuration (`partio.mcp.json` or the `PARTIO_*` environment variables), so Codex never has to carry it.

The Partio installer can also perform this step:

```bash
partio-mcp mcp install
```

Confirm the tools are visible in Codex after adding the server; if they are not, check that `partio-mcp` is on your `PATH`.
